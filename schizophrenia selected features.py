...
def rank_lasso(Xs, y):
    ...
    Features entering at the HIGHEST alpha (step 0) are most discriminative
    and selected consistently across folds → positive KI/JI values.
    Tie-break: larger |coef| at densest alpha.
    """
    y_f = y.astype(np.float64)
    alphas, coefs, _ = lasso_path(Xs, y_f, n_alphas=100,
                                   max_iter=5000, eps=1e-4)
    p_loc = coefs.shape[0]

    entry_step = np.full(p_loc, float(len(alphas)))
    for j in range(p_loc):
        nz = np.where(np.abs(coefs[j]) > 1e-10)[0]
        if len(nz):
            entry_step[j] = float(nz[0])

    final_coef = np.abs(coefs[:, -1])
    rank = np.lexsort((-final_coef, entry_step))
    return rank.copy()
